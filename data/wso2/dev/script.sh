...
echo oosv2 ... done
#echo curl http://is-int1-scala.api.scholastic.com:8080/remove -d "{\"name\":\"dw_subscriptions\",\"version\": \"1.0\"}" -H "Content-Type: application/json"
curl -X POST "http://is-int1-scala.api.scholastic.com:8080/publish" -H "Content-Type: application/json"  -d @oosv2.json

echo digital ... done
#echo curl http://is-int1-scala.api.scholastic.com:8080/remove -d "{\"name\":\"digitalDownload\",\"version\": \"1.0\"}" -H "Content-Type: application/json"
curl -X POST "http://is-int1-scala.api.scholastic.com:8080/publish" -H "Content-Type: application/json"  -d @digitalDownload.json
